...
function GitIntegrations() {
    ...
    const [newProviderModal, setNewProviderModal] = useState<{ type: string, host: string, errorMessage?: string } | undefined>(undefined);
    ...
    const yourProviderMenu: ContextMenuEntry[] = [
        {
            title: 'Edit',
            onClick: () => {
            }
    ...
    return (<div>
        ...
        <Modal visible={!!newProviderModal} onClose={() => setNewProviderModal(undefined)}>
            <h3>Add a Git Provider</h3>
            <div>
                <label htmlFor="type">Type</label>
                <select name="type" value={newProviderModal?.type}>
                    <option value="GitHub">GitHub</option>
                    <option value="GitLab">GitLab</option>
                </select>
            </div>
            <div>
                <label htmlFor="hostName">Git Provider's host name</label>
                <input name="hostName" type="text" value={newProviderModal?.host} />
            </div>
            <div>
        ...
        <div className="flex-grow flex flex-row pt-12">
            <h3 className="flex-grow self-center">Your Provider Integration</h3>
            <button className="self-center h-full" onClick={() => setNewProviderModal({ host: "gitlab.com", type: "GitLab" })}>Add New Provider</button>
        </div>
        <div className="flex flex-col pt-6 space-y-12">
            {providers && providers.map(ap => (
                <div key={"ap-" + ap.id} className="flex-grow flex flex-row">
                    <div key={"ap-" + ap.id} className="flex-grow flex flex-row hover:bg-gray-100 rounded-xl h-12">
                        <div className="flex-grow h-10 p-0 inline-flex my-auto">
                            <img className="fill-current filter-grayscale w-6 h-6 ml-3 mr-3 my-auto" src={iconForAuthProvider(ap.type)} />
                            <span className="pt-2 pb-2 mr-3 text-base my-auto font-medium truncate overflow-ellipsis">{simplifyProviderName(ap.host)}</span>
                        </div>
                        <div className="self-center">
                            <ContextMenu menuEntries={yourProviderMenu}>
                                <img className="w-8 h-8 p-1" src={ThreeDots} alt="Actions" />
                            </ContextMenu>
                        </div>
                    </div>
